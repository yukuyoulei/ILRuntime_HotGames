...
public class UBuildTools : EditorWindow
{
	...
	void StartBuild(string type)
	{
		...
		plistDocument.root.SetString("MOBAppKey", "274fa9936816e");
		plistDocument.root.SetString("MOBAppSecret", "8fa3f2e29924b70bd094b69cea3dd5ed");

		PlistElementDict dict = plistDocument.root.AsDict();
		dict.SetString("NSPhotoLibraryUsageDescription", photo);
		dict.SetString("NSCameraUsageDescription", "即将启用您的摄像头。");
		dict.SetString("NSMicrophoneUsageDescription", "即将启用您的麦克风。");

		/*PlistElementArray parray = plistDocument.root.CreateArray("CFBundleURLTypes");
		dict = parray.AddDict();
		dict.SetString("URL Schemes", "iqiyigame");
		dict.SetString("URL identifier", PlayerSettings.applicationIdentifier);*/
